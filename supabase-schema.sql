...
create table public.documents (
  ...
  order_index integer not null default 0,
  created_at timestamptz not null default now(),
  -- Which university this document is for; null = one copy covers all of them.
  target_university_id bigint references public.universities(id) on delete set null,
  review_comment text,
  review_file_url text,
  -- Standard-checklist metadata (see supabase-document-templates.sql).
  template_key text,
  lead_time_days integer,
  deadline_manual boolean not null default false,
  -- Soft delete, so seeding does not recreate what the student removed
  -- (see supabase-document-soft-delete.sql).
  deleted_at timestamptz
);

-- Seeding is idempotent via these: one shared copy per template, one per
-- university for the templates that are written per university.
...
-- Without this, RLS fails closed on DELETE and PostgREST reports the resulting
-- zero-row delete as a success. See supabase-delete-policies.sql.
